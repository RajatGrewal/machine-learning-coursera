...
% % disp('Size of yNew is : ');
% % disp(size(yNew));
% yVec = yNew;

% J = (1/m) * sum(sum((-yNew .* log(hThetax)) - (1-yNew) .* log( 1 - hThetax)));
...
% regularizationParameter = (lambda/(2*m)) * (sum(sum(Theta1(:,2:end).^2)) + sum(sum(Theta2(:,2:end).^2)));
...
% for k=1:m,
% 	a_1 = X(k,:);
% 	% disp('size of a_1 insisde the loop');
% 	% disp(size(a_1));
% 	z_2 = Theta1 * a_1';
% 	a_2 = sigmoid(z_2');
% 	% disp('size of a_2 insisde the loop');
% 	% disp(size(a_2));
% 	a_2 = [ones(1,1) a_2];
% 	z_3 = Theta2 * a_2';
% 	z_3 = z_3';
% 	% disp('size of z_3 insisde the loop');
% 	yy = ([1:num_labels]==y(k));
